    ...
    for a in alist:
        for b in blist:
            for lambda1 in lambdalist:
                distlist = []
                for ctx in ctxlist:
                    distlist.append(ctx.dist_beta_nc(a, b, lambda1));
                Title = 'dist=BetaNc, a=' + str(a) + ', b=' + str(b) \
                      + ', lambda1=' + str(lambda1)
                xlist = [1,2,3]
                xlist = [0.1]
...
def DemoPlotDistBetaNc():
    ...
    for j in range(len(a)):
        dlist.append(dreal.dist_beta_nc(a[j], b[j], lambda1[j]))
