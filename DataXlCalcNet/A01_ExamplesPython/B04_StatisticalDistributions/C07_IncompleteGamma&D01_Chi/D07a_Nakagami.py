...
def main_tests():
    DemoDistNakagami()
    DemoPlotDistNakagami()


def DemoDistNakagami():
    print('DemoDistNakagami: ');
    alist = [1.5, 3.5]
    blist = [5.1, 8.8]
    for a in alist:
        for b in blist:
            distlist = []
            for ctx in ctxlist:
                distlist.append(ctx.dist_nakagami(a, b));
            Title = 'dist=nakagami, a=' + str(a) + ', b=' + str(b)
            xlist = [1,2,3]
            xlist = [1]
...
def DemoPlotDistNakagami():
    ...
    for j in range(len(a)):
        dlist.append(dreal.dist_nakagami(a[j], b[j]))
