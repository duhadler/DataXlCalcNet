...
def main_tests():
    DemoDistStudentT()
    DemoPlotDistStudentT()


def DemoDistStudentT():
    print('DemoDistStudentT: ');
    nlist = [0.5, 1.5]
    for n in nlist:
        distlist = []
        for ctx in ctxlist:
            distlist.append(ctx.dist_student_t(n));
        Title = 'dist=StudentT, n=' + str(n)
        xlist = [1,2,3]
        xlist = [1]
...
def DemoPlotDistStudentT():
    ...
    for j in range(len(n)):
        dlist.append(dreal.dist_student_t(n[j]))
