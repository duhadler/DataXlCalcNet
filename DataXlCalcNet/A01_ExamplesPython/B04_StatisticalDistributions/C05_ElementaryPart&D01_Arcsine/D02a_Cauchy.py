...
def DemoDistCauchyCtx():
    print('DemoDistCauchyCtx: ');
    alist = [-1.5, 1.5]
    blist = [5.1, 8.8]
    for a in alist:
        for b in blist:
            distlist = []
            for ctx in ctxlist:
                distlist.append(ctx.dist_cauchy(a, b));
            Title = 'dist=Cauchy, a=' + str(a) + ', b=' + str(b)
            xlist = [1,2,3]
            xlist = [1]
...
def DemoPlotDistCauchy():
    ...
    xlim = [-40.0, 40.0]
    ylim = None
    if target=='qtf' or target=='isf': ylim=[-70, 70]

    dlist = []
    ltext = []
    for j in range(len(a)):
        dlist.append(dreal.dist_cauchy(a[j], b[j]))
