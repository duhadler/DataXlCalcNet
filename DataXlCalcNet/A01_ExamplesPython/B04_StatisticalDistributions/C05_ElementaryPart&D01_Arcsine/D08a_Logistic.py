...
def main_tests():
    DemoDistLogistic()
    DemoPlotDistLogistic()


def DemoDistLogistic():
    print('DemoDistLogistic: ');
    alist = [-1.5, 1.5]
    blist = [5.1, 8.8]
    for a in alist:
        for b in blist:
            distlist = []
            for ctx in ctxlist:
                distlist.append(ctx.dist_logistic(a, b));
            Title = 'dist=logistic, a=' + str(a) + ', b=' + str(b)
            xlist = [1,2,3]
            xlist = [1]
...
def DemoPlotDistLogistic():
    ...
    for j in range(len(a)):
        dlist.append(dreal.dist_logistic(a[j], b[j]))
