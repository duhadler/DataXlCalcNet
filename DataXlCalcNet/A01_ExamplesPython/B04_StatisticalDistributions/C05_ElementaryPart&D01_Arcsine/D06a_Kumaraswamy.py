...
def main_tests():
    DemoDistKumaraswamy()
    DemoPlotDistKumaraswamy()


def DemoDistKumaraswamy():
    print('DemoDistKumaraswamy: ');
    alist = [0.5, 1.5]
    blist = [5.1, 8.8]
    for a in alist:
        for b in blist:
            distlist = []
            for ctx in ctxlist:
                distlist.append(ctx.dist_kumaraswamy(a, b));
            Title = 'dist=Kumaraswamy, a=' + str(a) + ', b=' + str(b)
            xlist = [1,2,3]
            xlist = [1]
...
def DemoPlotDistKumaraswamy():
    ...
    for j in range(len(a)):
        dlist.append(dreal.dist_kumaraswamy(a[j], b[j]))
