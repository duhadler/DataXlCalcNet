...
def main_tests():
    DemoDistSkewNormal()
    DemoPlotDistSkewNormal()


def DemoDistSkewNormal():
    print('DemoDistSkewNormal: ');
    alist = [-1.5]
    blist = [5.1]
    clist = [1.5]
    for a in alist:
        for b in blist:
            for c in clist:
                distlist = []
                for ctx in ctxlist:
                    distlist.append(ctx.dist_skewnormal(a, b, c));
                Title = 'dist=skewnormal, a=' + str(a) + ', b=' + str(b) + \
                        ', c=' + str(c)
                xlist = [1,2,3]
                xlist = [1]
...
def DemoPlotDistSkewNormal():
    ...
    for j in range(len(a)):
        dlist.append(dreal.dist_skewnormal(a[j], b[j], c[j]))
        ltext.append('a=' + str(a[j]) + ', b=' + str(b[j]) + ', c=' + str(c[j]))
