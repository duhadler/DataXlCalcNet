...
def main_tests():
    DemoDistLognormal()
    DemoPlotDistLognormal()


def DemoDistLognormal():
    print('DemoDistLognormal: ');
    alist = [0.5, 1.5]
    blist = [5.1, 8.8]
    for a in alist:
        for b in blist:
            distlist = []
            for ctx in ctxlist:
                distlist.append(ctx.dist_lognormal(a, b));
            Title = 'dist=lognormal, a=' + str(a) + ', b=' + str(b)
            xlist = [1,2,3]
            xlist = [1]
...
def DemoPlotDistLognormal():
    ...
    for j in range(len(a)):
        dlist.append(dreal.dist_lognormal(a[j], b[j]))
