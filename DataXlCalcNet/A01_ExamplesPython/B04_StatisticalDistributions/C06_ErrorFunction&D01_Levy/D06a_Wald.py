...
def DemoDistWald():
    ...
    for mu in mulist:
        for b in blist:
            distlist = []
            for ctx in ctxlist:
                distlist.append(ctx.dist_wald(mu, b));
            Title = 'dist=wald, mu=' + str(mu) + ', b=' + str(b)
            xlist = [1,2,3]
            xlist = [1]
...
def DemoPlotDistWald():
    ...
    for j in range(len(mu)):
        dlist.append(dreal.dist_wald(mu[j], b[j]))
        ltext.append('mu=' + str(mu[j]) + ', b=' + str(b[j]))
