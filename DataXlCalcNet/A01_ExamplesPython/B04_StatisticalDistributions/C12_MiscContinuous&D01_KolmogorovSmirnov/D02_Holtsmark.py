...
def main_tests():
    DemoDistHoltsmark()
    DemoPlotDistHoltsmark()


def DemoDistHoltsmark():
    print('DemoHoltsmark: ');
    alist = [0.5, 1.5]
    blist = [5.1, 8.8]
    for a in alist:
        for b in blist:
            distlist = []
            for ctx in ctxlist:
                distlist.append(ctx.dist_holtsmark(a, b));
            Title = 'dist=beta, a=' + str(a) + ', b=' + str(b)
            xlist = [1,2,3]
            xlist = [0.1]
...
def DemoPlotDistHoltsmark():
    ...
    for j in range(len(a)):
        dlist.append(dreal.dist_holtsmark(a[j], b[j]))
