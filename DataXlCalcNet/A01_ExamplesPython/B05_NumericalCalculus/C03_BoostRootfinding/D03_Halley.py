...
def DemoHalleyCtx():
    ...
    for Ctx in boost_list:
        ...
        get_digits = Ctx.prec;
        maxit = 50;
        res = Ctx.Halley(Ctx.cb1SRet1S(F10), Ctx.cb1SRet1S(DF10), Ctx.cb1SRet1S(D2F10), guess, xmin, xmax, get_digits, maxit)
        print('Ctx.Halley(cb(f10), cb(df10), cb(d2f10), guess, xmin, xmax, get_digits, maxit): ')
        x0 = res.Item1
        iter1 = res.Item2
        print('x0:', Ctx.fmt(x0))
        print('iter1:', iter1)
